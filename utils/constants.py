...
# Trích dẫn và mở rộng từ seriaati/embed-fixer fixes.py (FixMethod domains).
# ---------------------------------------------------------------------------
PROXY_DOMAINS = {
...
#
# Proxy không có trong dict này sẽ được xác thực qua OG metadata.
# Trích dẫn từ seriaati/embed-fixer utils/fetch_info.py
# ---------------------------------------------------------------------------
PROXY_API_ENDPOINTS = {
    "fxtwitter.com": {
...
# Trích dẫn từ seriaati/embed-fixer utils/misc.py KEEP_QUERY_PARAMS.
# ---------------------------------------------------------------------------
KEEP_QUERY_PARAMS = {
    "youtube.com": {"v"},
}
...
def extract_urls(text: str) -> list[tuple[str, bool]]:
    ...
    Trích dẫn từ seriaati/embed-fixer utils/misc.py extract_urls().
    """
...
def domain_in_url(url: str, domain: str) -> bool:
    ...
    Trích dẫn từ seriaati/embed-fixer utils/misc.py domain_in_url().
    """
    parsed = urlparse(url)
...
def remove_query_params(url: str) -> str:
    ...
    Ví dụ: giữ ?v= cho YouTube, loại bỏ utm_source, fbclid, v.v.
    Trích dẫn từ seriaati/embed-fixer utils/misc.py remove_query_params().
    """
    parsed = urlparse(url)
...
def replace_domain(url: str, old_domain: str, new_domain: str) -> str:
    ...
    Trích dẫn từ seriaati/embed-fixer utils/misc.py replace_domain().
    """
    parsed = urlparse(url)
...
def sanitize_username(display_name: str) -> str:
    ...
    Trích dẫn từ seriaati/embed-fixer utils/misc.py sanitize_username().
    """
    if not display_name:
        return "User"
